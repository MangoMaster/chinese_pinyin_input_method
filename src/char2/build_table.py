...
def check_table(pinyin_char_table):
    """
    Check pinyin_char_table to be properly set.

    Args:
        pinyin_char_table: A dict, key->pinyin, value->[(char, log(probability))].
    """
    ...
    for key, value in pinyin_character_table.items():
        if not key in pinyin_char_table:
            pinyin_char_table[key] = [[value, 1], ]
            print("Miss: " + key + " Set to: " + value)
        else:
            max_char = ['', float("-inf")]
            for char, probability in pinyin_char_table[key]:
                if probability > max_char[1]:
                    max_char = (char, probability)
            assert max_char[0] != ''
            pinyin_character_table[key] = max_char[0]
    print(pinyin_character_table)
